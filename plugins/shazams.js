...
𝚁𝙴𝚂𝚄𝙻𝚃
• ☑️ *العنوان*: ${title}
• ☄️ *الفنانون*: ${artists !== undefined ? artists.map(v => v.name).join(', ') : 'لم يتم العثور'}
...
`.trim()
    ...
    m.reply(txt)
  } else throw '*قم أولا بإرسال أوديو فيه الأغنية أو الفيديو التي تريد البحث عنها بعدها أكتب* \n\n *.سماع*'
}

// تعيين بعض الخصائص للدالة handler
handler.help = ['سماع']
handler.tags = ['search']
handler.command = /^سماع|اسم|whatmusic$/i
